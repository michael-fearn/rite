...
class IngressGenerationTests(unittest.TestCase):
    def test_route_model_combines_service_and_host_routes_ordered_by_hostname(self):
        ...
        self.assertIn("dns-secondary.fearn.cloud", hostnames)
    ...
    def test_generated_routes_are_import_file_content_with_letsencrypt_dns_tls(self):
        ...
        self.assertIn("dns-secondary.fearn.cloud {\n\ttls {\n\t\tdns cloudflare {$CLOUDFLARE_API_TOKEN}\n\t}", caddy_routes)
        self.assertIn("\treverse_proxy http://10.40.0.18:8080", caddy_routes)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [
                    ...
                    "vm-shell dns-primary-vm -- sudo install -D -m 0644 /dev/stdin /srv/services/dns-primary/pihole/etc-dnsmasq.d/99-fortress-ingress.conf",
                    "stdin: # Generated by fortress ingress regeneration. Do not edit by hand.",
                    "vm-shell dns-primary-vm -- sudo systemctl restart fortress-dns-primary-pihole.service",
                    "vm-shell dns-secondary-vm -- sudo install -D -m 0644 /dev/stdin /srv/services/dns-secondary/pihole/etc-dnsmasq.d/99-fortress-ingress.conf",
                    "stdin: # Generated by fortress ingress regeneration. Do not edit by hand.",
                    "vm-shell dns-secondary-vm -- sudo systemctl restart fortress-dns-secondary-pihole.service",
                ],
                calls,
            )

    def test_ingress_regenerate_reports_successful_progress(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            shutil.copytree(REPO_ROOT / "inventory", root / "inventory")
            calls_log = root / "calls.log"
            ...
            self.assertEqual(
                [
                    "Regenerating ingress from Inventory...",
                    "running: push Caddy routes to internal-ingress-vm",
                    "ok: push Caddy routes to internal-ingress-vm",
                    "running: reload Caddy on internal-ingress-vm",
                    "ok: reload Caddy on internal-ingress-vm",
                    "running: push Ingress DNS records for dns-primary to dns-primary-vm",
                    "ok: push Ingress DNS records for dns-primary to dns-primary-vm",
                    "running: restart Pi-hole DNS for dns-primary on dns-primary-vm",
                    "ok: restart Pi-hole DNS for dns-primary on dns-primary-vm",
                    "running: push Ingress DNS records for dns-secondary to dns-secondary-vm",
                    "ok: push Ingress DNS records for dns-secondary to dns-secondary-vm",
                    "running: restart Pi-hole DNS for dns-secondary on dns-secondary-vm",
                    "ok: restart Pi-hole DNS for dns-secondary on dns-secondary-vm",
                    "Ingress regeneration complete.",
                ],
                result.stdout.splitlines(),
            )

    def test_dns_services_are_web_ui_routes_and_ingress_dns_targets(self):
        model = load_inventory_tree(REPO_ROOT)
        ...
        self.assertIn("reverse_proxy http://10.40.0.11:8080", caddy_routes)
        self.assertIn("dns-secondary.fearn.cloud {", caddy_routes)
        self.assertIn("reverse_proxy http://10.40.0.18:8080", caddy_routes)
        self.assertEqual(["dns-primary", "dns-secondary"], [target["service"] for target in targets])
        self.assertEqual("dns-primary-vm", targets[0]["backend_vm"])
        self.assertEqual("dns-secondary-vm", targets[1]["backend_vm"])
        self.assertEqual("pihole", targets[0]["provider"])
        self.assertEqual("pihole", targets[1]["provider"])
        ...
        secondary_published_ports = model.services["dns-secondary"]["deploy"]["containers"][0]["published_ports"]
        self.assertIn(
            {"bind": "10.40.0.18", "host": 53, "container": 53, "protocol": "tcp_udp"},
            secondary_published_ports,
        )
        self.assertIn(
            {"bind": "0.0.0.0", "host": 8080, "container": 80, "ingress": True, "protocol": "tcp"},
            secondary_published_ports,
    ...
    def test_ingress_dns_record_set_renders_declared_routes_to_ingress_address_only(self):
        ...
        self.assertEqual(
            [
                ...
                "address=/prowlarr.fearn.cloud/10.40.0.16",
                "address=/qbittorrent.fearn.cloud/10.40.0.16",
                "address=/radarr.fearn.cloud/10.40.0.16",
                "address=/sabnzbd.fearn.cloud/10.40.0.16",
                "address=/seerr.fearn.cloud/10.40.0.16",
                "address=/sonarr.fearn.cloud/10.40.0.16",
                "address=/straylight.fearn.cloud/10.40.0.16",
                "address=/wintermute.fearn.cloud/10.40.0.16",
            ],
            record_sets[0]["content"].splitlines(),
        )
        self.assertEqual(record_sets[0]["content"], record_sets[1]["content"])
